...
def read_dataset(folder):
    ...
    # sup_ent_pairs_list = list(sup_ent_pairs)[:100]
...
def merge_seed(c_dict, str_tmp_dict, att_tmp_dict, aligned_test_pairs):
    ...
    # for i, j in str_tmp_dict.items():
    #     if att_tmp_dict.get(i) != None:
    #         att_id = att_tmp_dict[i][0]
    #         str_id = j[0]
    #         att_ratio = round(att_tmp_dict[i][1]/att_len, 6)
    #         str_ratio = round(j[1]/str_len, 6)

    #         if att_id != str_id:
    #             if str_ratio > att_ratio: 
    #                 merge_dict[i] = (att_tmp_dict[i][0], att_ratio)
    #             else:
    #                 merge_dict[i] = (j[0], str_ratio)
            
    #         else:
    #             merge_dict[i] = (j[0], min(att_ratio, str_ratio))
